...
<div style="padding:50px; padding-left:6%;  padding-right:6%">
      ...
      <p style="text-align:center; font-size:20px; font-weight:bold">View All Send Emails to Subscribers</p>
      ...
      <?php
            foreach($subscriberes as $user) { 
      ?>
      <tr>
      <td><?=$user['category_name']?></td>
      <td><?=$user['subscribe_email']?></td>
      <td><?=$user['system_datetime']?></td>
      <td><a href="<?=base_url('Subscribers/viewEmail/'.$user['id'])?>" class="pull-right btn"><i style="margin-right:8px;" class="fa fa-eye "></i> View </a</td>
      </tr>
      <?php } ?>
